...
enum class WebSocketOpcode { Continuation = 0x00, Text = 0x01, Binary = 0x02, Close = 0x08, Ping = 0x09, Pong = 0x0A };
...
void WebSocketClient::Connect(const std::string& url, std::optional<std::string> const& token) {
...
void WebSocketClient::Connect(
    ...
    if (token && !token.value().empty()) {
        handshakingWords.append("Authorization: Bearer ").append(token.value()).append("\r\n");
    }
...
void WebSocketClient::OnTextReceived(Callback<std::string> callback) { TextReceivedCallback = callback; }

void WebSocketClient::OnBinaryReceived(Callback<std::vector<uint8_t>> callback) { BinaryReceivedCallback = callback; }

void WebSocketClient::OnError(Callback<std::string> callback) { ErrorCallback = callback; }

void WebSocketClient::OnLostConnection(Callback<int> callback) { LostConnectionCallback = callback; }

void WebSocketClient::SendText(const std::string& text) {
    if (status == Status::Closed) throw std::runtime_error("WebSocket is closed.");
...
void WebSocketClient::Pong(const std::vector<uint8_t>& data) {
    if (status == Status::Closed) return;
            ...
            if (offset < 0) {
                ...
                if (buffer.size() >= offset + info.PayloadLength) {
                    ...
                    if (info.Opcode == WebSocketOpcode::Text) {
                        std::string text = std::string(payload_start, payload_end);
                        ...
                        if (BinaryReceivedCallback != nullptr) {
                            BinaryReceivedCallback(*this, std::vector<uint8_t>(payload_start, payload_end));
                        }
                    } else if (info.Opcode == WebSocketOpcode::Ping) {
                        try {
                            Pong(std::vector<uint8_t>(payload_start, payload_end));
                        } catch (const std::exception& ex) {
                            if (ErrorCallback != nullptr) {
                                ErrorCallback(
                                    *this,
                                    std::string("An error occurs on sending pong frame. error: ") + ex.what()
                                );
                            }
                        }
                        ...
                        if (ErrorCallback != nullptr) {
                            ErrorCallback(
                                *this,
                                std::string("The opcode #")
                                    .append(std::to_string((int)info.Opcode).append(" is not supported."))
                            );
                        }
                    }
